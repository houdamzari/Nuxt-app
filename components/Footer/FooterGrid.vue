<template>
  <div class="flex flex-row w-full h-full justify-between px-32 mb-10">
    <div class="flex flex-col gap-16">
      <h1 class="text-xl font-bold">HOME PAGE</h1>
      <div class="flex flex-col gap-10">
        <p class="text-grey">Content Writing</p>
        <p class="text-grey">Start Course</p>
        <p class="text-grey">Do something</p>
        <p class="text-grey">Lien</p>
        <p class="text-grey">Lien</p>
      </div>
    </div>
    <div class="flex flex-col gap-16 ">
      <h1 class="text-xl  font-bold">MAIN LINKS</h1>
      <div class="flex flex-col gap-10">
        <p class="text-grey">Content Writing</p>
        <p class="text-grey">Start Course</p>
        <p class="text-grey">Do something</p>
        <p class="text-grey">Lien</p>
        <p class="text-grey">Lien</p>
      </div>
    </div>
    <div class="flex flex-col gap-16">
      <h1 class="text-xl  font-bold">FOR USERS</h1>
      <div class="flex flex-col gap-10">
        <p class="text-grey">Content Writing</p>
        <p class="text-grey">Start Course</p>
        <p class="text-grey">Do something</p>
        <p class="text-grey">Lien</p>
        <p class="text-grey">Lien</p>
      </div>
    </div>
    <div class="flex flex-col gap-16">
      <h1 class="text-xl  font-bold">Resources</h1>
      <div class="flex flex-col gap-10">
        <p class="text-grey">Content Writing</p>
        <p class="text-grey">Start Course</p>
        <p class="text-grey">Do something</p>
        <p class="text-grey">Lien</p>
        <p class="text-grey">Lien</p>
      </div>
    </div>
    <div class="flex flex-col gap-16">
      <h1 class="text-xl  font-bold">ALL OUR PRODUCTS</h1>
      <div class="flex flex-col gap-10">
        <p class="text-grey">Content Writing</p>
        <p class="text-grey">Start Course</p>
        <p class="text-grey">Do something</p>
        <p class="text-grey">Lien</p>
        <p class="text-grey">Lien</p>
      </div>
    </div>
    <div class="flex flex-col gap-16 mt-6">
      <h1 class=""></h1>
      <div class="flex flex-col gap-10">
        <p class="text-grey">Content Writing</p>
        <p class="text-grey">Start Course</p>
        <p class="text-grey">Do something</p>
        <p class="text-grey">Lien</p>
        <p class="text-grey">Lien</p>
      </div>
    </div>
  </div>
</template>

<script>
export default {};
</script>

<style scoped></style>
